...
def test_init_network():
    network = Network()
    network.add_reaction_from_file("test/test_input/deuspin.kida.uva.2017.in", "kida")
    network.check_duplicate_reaction()


def test_check_duplicate():
    network = Network()
    network.add_reaction_from_file("test/test_input/duplicate_test.dat", "kida")
    network.check_duplicate_reaction(full_check=False)


def test_generate_cvode_code_from_kida():
    network = Network()
    network.add_reaction_from_file("test/test_input/simple_test.dat", "kida")
    network.check_duplicate_reaction()
...
def test_generate_cvode_code_from_leeds():
    network = Network()
    network.add_reaction_from_file("test/test_input/rate12_twobody_HO.rates", "leeds")
    network.check_duplicate_reaction()
...
def test_generate_cvode_code_from_krome():
    network = Network()
    network.add_reaction_from_file("test/test_input/react_primordial.krome", "krome")
